...
function PlayState:enter(params)
    ...
    self.spawnPowerup = 15
    self.hits = 0
    
    self.lockedBricks = false

    for k, brick in pairs(self.bricks) do
        if brick.locked == true then
            self.lockedBricks = true
        end
    end
...
function PlayState:update(dt)
    ...
    for i=1, table.getn(self.balls) do
        self.balls[i]:update(dt)
    end    

    -- updates powerup if it's in play
    if self.powerup.inPlay then
        self.powerup:update(dt)
    end

    -- checks if the powerup has hit the paddle or left the screen
    if self.powerup:collides(self.paddle) then
        self.powerup.inPlay = false
        self.powerup.x = math.random(8, VIRTUAL_WIDTH - 24)
        self.powerup.y = VIRTUAL_HEIGHT / 3 - 8
        self.hits = 0
        if self.powerup.version == 9 then
            for i=2, 3 do
        ...
        elseif self.powerup.version == 10 then
            for k, brick in pairs(self.bricks) do
                if brick.locked == true then
                    brick.locked = false
                end
    ...
    -- spawns the powerup
    if self.hits >= self.spawnPowerup and not self.powerup.inPlay  then
        if self.lockedBricks == true then
            self.powerup.version = 10
            self.powerup.inPlay = true
            self.hits = 0
        elseif table.getn(self.balls) == 1 then
            self.powerup.version = 9
            self.powerup.inPlay = true
            self.hits = 0
        end
    end
            ...
            if brick.inPlay and self.balls[i]:collides(brick) then
                ...
                if brick.locked == false then
                    ...
                    if self.score > self.recoverPoints then
                        ...
                        self.health = math.min(3, self.health + i)
                    ...
                    if self.score > self.increaseSizePoints then
                        ...
                        self.increaseSizePoints = math.min(100000, self.increaseSizePoints * 2)
                    end
                end
